        ...
        Expanded(
          child: ListView(
            ...
            children: [
                  ...
                  child: ListTile(
                    ...
                    onTap: () async {
                      ...
                      // Ignore setState, it will rebuild on next visit
                            ...
                            // ── Floating Assistant ──────────────────────────
                            _sectionHeader(context, 'Floating Assistant'),
                            const SizedBox(height: 12),
                            _card(
                              context,
                              child: ListTile(
                                ...
                                subtitle: Text(
                                  'Chat, upload & screen-analysis from any app',
                                  style: TextStyle(color: context.textD, fontSize: 12),
                                ),
                                onTap: () {
                                  Get.snackbar('Floating Bubble', 'Requesting permission...');
                                  OverlayService.show().then((_) {
                                    Get.snackbar('Floating Bubble', 'Show() completed');
                                  }).catchError((e) {
                                    Get.snackbar('Floating Bubble Error', e.toString());
                                  });
                                },
                                ),
                                ),
              ...
              _sectionHeader(context, 'Remote Laptop Server'),
              const SizedBox(height: 8),
              Text(
                'Offload heavy tasks (like image analysis) to a laptop running Ollama on the same WiFi network.',
                style: TextStyle(fontSize: 12, color: context.textD),
              ),
              const SizedBox(height: 12),
...
class _RemoteServerCardState extends State<_RemoteServerCard> {
  ...
  void initState() {
    super.initState();
    _urlController = TextEditingController(text: widget.storage.remoteServerUrl);
    _modelController = TextEditingController(text: widget.storage.remoteModelName);
  }
  ...
  Future<void> _testConnection() async {
    ...
    widget.storage.remoteServerUrl = _urlController.text.trim();
    final remote = Get.find<RemoteInferenceService>();
    final ok = await remote.checkReachability();
    setState(() {
      _testing = false;
      _lastTestResult = ok;
    });
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: 'http://10.230.106.40:11434',
              labelStyle: TextStyle(color: context.textM),
              hintStyle: TextStyle(color: context.textD),
              filled: true,
              fillColor: context.bgInput,
              ...
              focusedBorder: OutlineInputBorder(
                ...
                borderSide: const BorderSide(color: AppColors.accent),
                    ...
                    TextField(
                      ...
                      onChanged: (val) => widget.storage.remoteModelName = val.trim(),
                    ),
          const SizedBox(height: 12),
          StatefulBuilder(
            builder: (context, setInnerState) {
              return SwitchListTile(
                title: Text('Use laptop when available',
                    style: TextStyle(color: context.text, fontSize: 14)),
                value: widget.storage.useRemoteWhenAvailable,
                onChanged: (val) {
                  widget.storage.useRemoteWhenAvailable = val;
                  setInnerState(() {});
                },
                activeThumbColor: AppColors.accent,
                contentPadding: EdgeInsets.zero,
          ...
          if (_lastTestResult != null) ...[
            ...
            Row(
              children: [
                Icon(
                  _lastTestResult! ? Icons.check_circle_rounded : Icons.error_rounded,
                  size: 16,
                  color: _lastTestResult! ? AppColors.green : AppColors.red,
                ),
                const SizedBox(width: 6),
                Text(
                  _lastTestResult! ? 'Connected!' : 'Could not reach laptop',
                  style: TextStyle(
                    color: _lastTestResult! ? AppColors.green : AppColors.red,
                    fontSize: 12,
                  ),
