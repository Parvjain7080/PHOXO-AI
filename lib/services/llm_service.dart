...
class LlmService extends GetxService {
  ...
  Future<void> loadModel(String path) async {
    ...
    try {
      ...
      final contextSize = Platform.isAndroid ? 2048 : 2048;
      ...
      // Optimize threads: 4 for both generation and batch processing to keep memory stable.
            // Use up to 4 threads, but never 0 — a device with 4 or fewer
            // cores should still use all of them, not fall back to 0.
            final threadCount = Platform.numberOfProcessors >= 4
                ? 4
            ...
            final params = ModelParams(
              ...
              numberOfThreadsBatch: threadCount,
            );

            log?.info('Backend=$parsedBackend, GPU layers=$userGpuLayers, ctx=$contextSize, threads=$threadCount', source: 'LLM');
  ...
  Future<bool> checkSupportsVision() async {
    if (_engine == null) return false;
    return await _engine!.supportsVision;
  }
  /// Load the vision projector alongside an already-loaded model.
  Future<void> loadMultimodalProjector(String mmprojPath) async {
    if (_engine == null) throw StateError('Load a model before the projector.');
    await _engine!.loadMultimodalProjector(mmprojPath);
  }

  /// Generate a response using an image + text prompt.
  Stream<String> generateWithImage({
    ...
    try {
      ...
      await for (final chunk in _engine!.create(messages)) {
        final content = chunk.choices.isNotEmpty
