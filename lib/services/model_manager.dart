...
class ModelManager extends GetxService {
  ...
  Future<ModelManager> init() async {
    ...
    // ── Vision projector (mmproj) linking ─────────────────────────
    Map<String, String> get _mmprojMap {
      final box = Hive.box('models_meta');
      final raw = box.get('mmproj_map', defaultValue: <String, dynamic>{});
      return Map<String, String>.from(raw as Map);
    }

    String? getMmprojFilename(String modelFilename) => _mmprojMap[modelFilename];

    String? getMmprojPath(String modelFilename) {
      final mmprojFile = getMmprojFilename(modelFilename);
      if (mmprojFile == null) return null;
      return p.join(_modelsDir, mmprojFile);
    }

    Future<void> linkMmproj(String modelFilename, String mmprojFilename) async {
      final box = Hive.box('models_meta');
      final map = _mmprojMap;
      map[modelFilename] = mmprojFilename;
      await box.put('mmproj_map', map);
    }

    /// Move an mmproj file into the models folder WITHOUT adding it to
    /// downloadedModels (it's not a chat model itself, just a helper file).
    Future<void> importMmprojFile(String sourcePath, String mmprojFilename) async {
      final destPath = p.join(_modelsDir, mmprojFilename);
