...
class ModelController extends GetxController {
  ...
  void onInit() {
    ...
    /// Pick and link a vision projector (mmproj) file to a specific model.
    Future<void> importMmprojForModel(String modelFilename) async {
      try {
        ...
        if (!name.endsWith('.gguf')) {
          Get.snackbar('Invalid File', 'Please select the mmproj .gguf file.',
              snackPosition: SnackPosition.BOTTOM);
          return;
        }
        if (file.path == null) return;

        await _manager.importMmprojFile(file.path!, name);
        await _manager.linkMmproj(modelFilename, name);

        Get.snackbar('Vision Enabled', 'Projector linked — reload the model to use it.',
            snackPosition: SnackPosition.BOTTOM);
      } catch (e) {
    ...
    try {
      ...
      if (!_llm.isLoaded.value && loadingModelFilename.value == null) {
        return; // Was cancelled
            ...
            // If a vision projector is linked to this model, load it too
            final mmprojPath = _manager.getMmprojPath(filename);
            if (mmprojPath != null && await File(mmprojPath).exists()) {
              try {
                await _llm.loadMultimodalProjector(mmprojPath);
              } catch (e) {
                LogService? log;
                try { log = Get.find<LogService>(); } catch (_) {}
                log?.error('Failed to load vision projector: $e', source: 'Model');
              }
            }
    } catch (e) {
